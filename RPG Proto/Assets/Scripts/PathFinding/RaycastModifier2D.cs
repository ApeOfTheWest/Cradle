...
//2D version of the raycast simplifier modifier
namespace Pathfinding {
	 ...
	 */
	[AddComponentMenu("Pathfinding/Modifiers/Raycast Simplifier 2D")]
	[RequireComponent(typeof(Seeker))]
	[System.Serializable]
	[HelpURL("http://arongranberg.com/astar/docs/class_pathfinding_1_1_raycast_modifier.php")]
	public class RaycastModifier2D : MonoModifier {
		#if UNITY_EDITOR
		...
		public bool ValidateLine (GraphNode n1, GraphNode n2, Vector2 v1, Vector2 v2) {
			if (useRaycasting) {
				// Use raycasting to check if a straight path between v1 and v2 is valid
				if (thickRaycast && thickRaycastRadius > 0) {
					if (Physics2D.CircleCast (v1 + raycastOffset, thickRaycastRadius, v2 - v1, (v2 - v1).magnitude, mask)) {
						return false;
					} 
				} else {
